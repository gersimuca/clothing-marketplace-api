...
public class GarmentService {
    ...
    public List<GarmentDto> findAll(final GarmentType type, final GarmentSize size, final BigDecimal minPrice, final BigDecimal maxPrice ){
        return garmentRepository.findAll((root, query, criteriaBuilder) -> {
        ...
        }).stream().map(garmentMapper::mapToDto).collect(Collectors.toList());
    ...
    public GarmentDto publishGarment(final GarmentDto garmentDto, final UserEntity publisher) {
        ...
        final UserEntity user = userRepository
                ...
                .orElseThrow(() -> new EntityNotFoundException(UserEntity.class, publisher.getUsername()));

        GarmentEntity garment = GarmentEntity
                .builder()
                .type(type)
    ...
    public GarmentDto updateGarment(final Long id, final GarmentDto garmentDto, final UserEntity publisher) {
        final GarmentType type = garmentDto.getType();
        final String description = garmentDto.getDescription();
        final GarmentSize size = garmentDto.getSize();
        final BigDecimal price = garmentDto.getPrice();

        GarmentEntity entity = garmentRepository.findById(id).orElseThrow(() -> new EntityNotFoundException(GarmentEntity.class, id));
    ...
    public void deleteGarment(final Long id, final UserEntity publisher) {
        GarmentEntity garment = garmentRepository.findById(id).orElseThrow(() -> new EntityNotFoundException(GarmentEntity.class, id));
